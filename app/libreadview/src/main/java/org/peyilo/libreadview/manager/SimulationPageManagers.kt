...
class SimulationPageManagers private constructor() {
    ...
    class Test: FlipOnReleaseLayoutManager.Horizontal(), AnimatedLayoutManager {
    ...
    class Style2: FlipOnReleaseLayoutManager.Horizontal(), AnimatedLayoutManager {
        ...
        override fun prepareAnim(initDire: PageDirection) {
            ...
            renderer.setPageSize(pageContainer.width.toFloat(), pageContainer.height.toFloat())
            renderer.setPages(pageBitmap.topBitmap!!, pageBitmap.bottomBitmap!!)
            renderer.initDownPosition(gesture.down.x, gesture.down.y)
        }

        override fun startNextAnim() {
            renderer.flipToNextPage(scroller, animDuration)
            isAnimRuning = true
            pageContainer.invalidate()
        }

        override fun startPrevAnim() {
            renderer.flipToPrevPage(scroller, animDuration)
            isAnimRuning = true
            pageContainer.invalidate()
        }
        ...
        override fun onDragging(
            ...
            renderer.updateTouchPosition(gesture.cur.x, gesture.cur.y)
            pageContainer.invalidate()
        }
        ...
        override fun computeScroll() {
            if (scroller.computeScrollOffset()) {
                renderer.updateTouchPosition(scroller.currX.toFloat(), scroller.currY.toFloat())
                pageContainer.invalidate()
                // 滑动动画结束
                if (scroller.currX == scroller.finalX && scroller.currY == scroller.finalY) {
                    scroller.forceFinished(true)
